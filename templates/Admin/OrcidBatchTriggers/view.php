...
<div class="row">
    <div class="column-responsive column-80">
        <div class="orcidBatchTriggers view content">
            <div>The User must be at the Workflow Checkpoint for Trigger Delay days and not have already been sent the
                Email Batch (or must have been sent the Email Batch at least Repeat Every days ago if Repeat Every is
                non-zero). The User must match the Group criteria (if provided), and the User must have already received
                a prior email as specified by Require Prior Batch. Today's date must be after the Begin Date (if
                provided). The number of times this Email Batch has been sent to this user must not repeat more than
                Repeat Limit times (if provided).</div>
            ...
            <table>
                ...
                <tr>
                    <th><?= __('Group') ?></th>
                    <td><?= $orcidBatchTrigger->has('orcid_batch_group') ? $this->Html->link($orcidBatchTrigger->orcid_batch_group->NAME, ['controller' => 'OrcidBatchGroups', 'action' => 'view', $orcidBatchTrigger->orcid_batch_group->ID]) : '' ?>
                    </td>
                </tr>
                <tr>
